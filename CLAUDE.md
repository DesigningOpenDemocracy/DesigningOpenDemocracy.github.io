...
- `hooks/citation_export.py` — fires on `on_pre_build`; exports all event and footnote citations (`quote:` + `url:`) to `/data/citations.json` in CSL-JSON format with DOD content-integrity extension fields (`dod-quote`, `dod-content-sha256`, `dod-last-verified-date`). Reads `util/.event_evidence_cache.json` for hashes and verified dates. See `internal-heartbeat/dod-citation-standard.md` for the design.
