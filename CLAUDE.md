...
- Before pushing: `make check-links && python util/check_event_sourcing.py && python util/check_elections.py && python util/reorder_frontmatter.py --check && python util/check_footnote_quotes.py` — catches the same errors as CI. **Use `make check-links`, not `make build`**: CI runs `mkdocs build --strict` (`.github/workflows/build.yml`), which fails on broken-link warnings that a plain `mkdocs build` only logs as INFO and lets through; `make check-links` runs the `--strict` build plus `util/check_internal_links.py` in one step, `make build` does neither. (This line used to say `make build` and claim it "catches the same errors as CI" — it didn't: two commits landed on `main` with a broken link `make build` passed clean on, caught only by CI's actual strict-mode run. See HEARTBEAT.md's Step 7 for the incident.) The pre-commit hook (`.githooks/pre-commit`) auto-runs `reorder_frontmatter.py` (which fixes frontmatter ordering in place) on staged org pages, so the `--check` should always pass — it's a safety net. `check_footnote_quotes.py` also runs in the hook on any staged docs page, but it can't auto-fix a missing justification the way `reorder_frontmatter.py` can reorder fields — it blocks the commit until a quote or an `unquoted:` annotation is added by hand.
- The hook runs its checks with `venv/bin/python` when the repo has a `venv/` (the same environment `make` uses), falling back to `python3`. It used to call a bare `python`, which on a machine without `python-frontmatter` in the system Python refused every commit touching an org page with no error message; failures now print the script's output.
...
- **Curation standard**: An org belongs here if it works on systems of governance for/with the people, in good faith — regardless of ideological label. See `docs/projects/accountability-framework/index.md` for the full framework, including the three disqualifiers (hypocrisy, bad faith, structural inflexibility). DOD is not a human rights observatory; orgs focused purely on documenting abuses without engaging governance design do not fit.
  - **This is a necessary gate, not a sufficient one — a second, separate check is whether the entry actually serves DOD's readership.** The framework answers a universal legitimacy question ("is this system genuinely trying to govern for and with its people, in good faith") the same way for Russia, China, and Australia alike, and is deliberately not shaped by audience appeal — that consistency is what makes it trustworthy across ideologically hostile cases. Whether a *specific* org is worth a page *right now*, given limited curatorial attention, is a different and more mundane question: would someone in the democracy/governance-reform space actually click through and get value from this, or is its connection to that audience thin even though it's technically eligible? An org can clear the framework on a generous reading and still not clear this bar — e.g. a peak advocacy body for a housing model whose self-governance angle is one feature of what it promotes, rather than governance-mechanism work in its own right, may be a weaker case for a page than its framework-eligibility alone suggests. Keep the two checks separate rather than folding "is this useful to our readers" into the framework document itself: the framework's job is to stay a stable, universal standard, and a readership-appeal clause would make it start bending to exactly the kind of local, contingent judgment it's designed to resist. Track a borderline case that fails this second check in `internal-heartbeat/orgs-not-included.md`'s weak-fit watch list rather than either adding a page or discarding the research.
  - **The split itself generalises beyond DOD, even though the specific audience named above doesn't.** A universal legitimacy test (does this system genuinely govern for and with its people, in good faith) paired with a separate, local relevance test (is this worth a page to *this* project's actual readers, given *this* project's actual curatorial capacity) is a pattern any similarly-curated resource could reuse — including one with a different audience, different available research time, or a different region of focus than DOD's Australia-based, democracy-reform-space readership. The legitimacy half should port unchanged; the relevance half is exactly the part that's supposed to be swapped out per project rather than copied verbatim — so if this repo's approach is ever adapted elsewhere, that second gate is the one to rewrite for the new project's own readers and constraints, not the one to treat as a fixed standard alongside the framework.
...
- `events: [{date, title, short_title, url, source, note, quote, proof_level, proof_level_locked, proof_warning, url_checked, end_date, time, end_time, notable, notable_reason}]` — optional; a manually curated, editorial list of an org's significant milestones — **not** derived from `ics_feed:` and not meant to mirror it. `hooks/org_events.py` splits entries at build time into `page.meta.upcoming_events` (date >= today) and `page.meta.history_events` (date < today), rendered by `organisation.html` as two timeline sections. Keep each entry to one line — a terse `title`, no prose paragraph; if a milestone needs real narrative, put that in the page body instead (same judgment call as the optional "Key people" section). Future-dated entries here are also picked up by the site-wide calendar (see below) as a `manual`-source candidate alongside `ics_feed`-synced ones — no separate declaration needed. `end_date:` is optional, for events spanning more than one day (inclusive last day — same convention `util/sync_events.py` uses for iCal `DTEND`). `notable:` is optional and three-valued — absent/`false` (the default), `"medium"`, or `true` (major) — controlling the calendar's highlight tier:
  ...
  - **Notable events (`notable: true` or `notable: "medium"`) need *mechanical* proof** — a `quote:`, not just a `note:`. This is a separate, softer check (printed as `NOTABLE NO PROOF`, does not fail the build) reflecting that a claim flagged as either tier of `notable` deserves independently-verifiable evidence, not just an editor's paraphrase.
...
3. **Site-wide future calendar** (`docs/calendar.md`, template `calendar.html`) — a forward-looking, cross-org aggregate meant to help people find events to attend, not an archive. Built from three future-only sources merged by `hooks/calendar_export.py`:
   ...
   Three additive, opt-in refinements on top of the base date filter — none of them hides anything, so they don't conflict with the "no importance filtering" rule above:
   ...
   - **"Major event" / "Notable" highlighting** — `notable: true` gets the calendar's strongest highlight (amber card, larger title, a "★ Major event" badge); `notable: "medium"` gets a distinct, deliberately quieter blue card and a "● Notable" badge, for a real event that doesn't clear the major bar (see the `events:` `notable:` docs above for the actual line between the two). Deliberately **not** available on raw `ics_feed` syncs — a synced feed has no "this one matters more" signal to key off, only the curated `events:` source does.
   - **Expandable per-event description** — an event carrying `note:` and/or `quote:` renders a collapsed `<details class="calendar-event-detail">` ("More about this event") holding the note as a paragraph and the quote as a blockquote. This mirrors the `<details>` `organisation.html` already wraps the same two fields in, and is the place to put a fuller description of an event than its one-line `title:` should carry — `notable_reason:` stays a short caption, not a description. Collapsed by default so a long note can't push the following events off the screen. Like the tiers above it's curated-only: a raw `ics_feed` sync has no `note:`/`quote:` to show. Until 2026-09-17 `_load_manual_events()` simply never read these two fields, so an event's only human-readable description reached the org's own timeline and nowhere else, and a reader browsing `/calendar/` saw a bare title — `tests/test_calendar_export.py` pins the passthrough, since dropping a field there is silent (the calendar still builds).
...
### Election dates (`docs/data/elections.yml`)

Polling days are the one thing on the site-wide calendar that belongs to no
organisation, so they can't live in an org page's `events:` frontmatter the
way every other calendar entry does. They live in one curated YAML file,
merged in by `hooks/calendar_export.py` as the calendar's third source, and
rendered with their own violet card and 🗳️ badge — a different *kind* of
entry, deliberately not competing with `notable:`'s major/medium tiers,
which are about importance.

- **Scope**: Australian elections at every level (federal, state/territory,
  local) aim to be complete — DOD is Australian-based and these are the
  dates our readers actually vote on. Everything else is a curated
  selection of national elections plus a few subnational votes of clear
  interest to this landscape (Taiwan's local elections, where two tracked
  orgs do their work), not a world index — Wikipedia's "List of elections
  in \<year\>" already does the complete version better than we could
  maintain. **Listing an election is a statement about the date, not about
  the election** — nothing here reads as DOD judging a contest free, fair,
  or competitive.
- **An election with no date doesn't go in.** The Tasmanian state election
  expected in 2029 is the standing example: a calendar entry needs a day,
  and inventing one is worse than the omission.
- **Once an election is held, replace its entry with the next one for that
  jurisdiction** rather than accumulating history in this file. The calendar
  drops past entries either way, so a stale one is invisible rather than
  wrong; `check_elections.py` prints it as a `PAST` warning.
- **Schema**: `date` (required), `end_date`, `country` (ISO 3166-1 alpha-2,
  and it must be one `calendar_export.py`'s `_COUNTRY_NAMES` knows, or the
  entry renders as a bare code with no flag), `jurisdiction`, `level`
  (`national` | `state` | `local` | `supranational`), `title`,
  `date_status`, `date_note`, `url` **or** `source`, `quote` **or** `note`,
  `url_checked`. The `url`/`source` and `quote`/`note` pairs are the same
  two hard gates org `events:` carry, for the same reason.
- **`date_status:` is what makes this honest.** Election dates are often
  genuinely unsettled, and an unqualified date on a calendar reads as a
  settled appointment:
  - `fixed` — the polling day is set, by statute or by an issued writ. No
    caveat rendered.
  - `expected` — the date follows from a rule (a fixed term, a "fourth
    Saturday in August" clause) but hasn't been formally called. Renders
    "expected date".
  - `deadline` — not called; this is the *last lawful day* it can be held.
    Renders "due by this date". The next Australian federal election is
    this case (20 May 2028 for a simultaneous House and half-Senate
    election).
  - `month` — only the month is published, typically a postal ballot run
    across weeks rather than on one day (the 2026 Tasmanian local
    elections). The entry carries the first of that month and renders
    "day not yet set".

  Anything other than `fixed` should carry a `date_note:` saying where the
  date comes from — the linter warns when it doesn't, and the note renders
  as the card's caption line. The qualifier also rides into the `.ics`
  SUMMARY, since a subscriber sees that line with none of the page's badges
  around it.
- **Verification**: election `quote:`s are checked against their live pages
  by `util/check_fragments.py` in the same weekly cron pass as event and
  footnote quotes (`--elections-only` scopes a run to just these; `--slug`
  is org-scoped and skips them). Prefer quoting a sentence that states the
  date inline — the same preference event quotes have, and here it's the
  whole point of the citation.
- **Feeds**: elections appear in `docs/calendar.ics` and the per-country
  `docs/calendar-<CC>.ics` feeds like anything else, plus their own
...
- `hooks/org_events.py` — fires on `on_page_context`; splits a single org's `events:` frontmatter into `page.meta.upcoming_events` / `page.meta.history_events` for that org's own page timeline. Also fires on `on_env` to register two Jinja filters: `with_fragment` (from `util/text_fragment.py`), which `organisation.html` uses to derive each event's `#:~:text=` link at build time from `quote:` — see Calendar section below — and `archive_info_for`, which looks up a citation url's recorded Wayback snapshot/`url_status` for the archive-link rendering described in "Citation archival" above. The same `on_env` also builds the `org_event_highlight` Jinja global (slug → one display-ready event) that the Democracy Landscape index's quick-profile cards render — see `select_highlight()` and the "Quick-profile cards" bullet under Org index page below. It is a whole-corpus pass for the same reason `activity_selector.py`'s `org_activity_cache` is (the index renders in the same pass as every org page, so it can't read their `on_page_context` output), but it reads `files` rather than re-loading frontmatter from disk: MkDocs populates every page's `meta` *before* firing `on_env` (`mkdocs/commands/build.py` — `_populate_page` for all files, then `on_env`, then `_build_page`), which is also why the index template can already read `file.page.meta` for other org pages.
...
- `.org-info-btn` / `.org-info-card` / `.org-info-*` — the ℹ️ quick-profile trigger in the org index's Name column, and the card it opens. **The card is moved to `<body>` by the page script and therefore renders outside `.md-typeset`**, so unlike almost everything else in this file its rules are bare classes and every inherited typeset default (font size, spacing, link colour) is restated explicitly — don't "fix" those selectors by scoping them. The move is not cosmetic either: `.org-table-scroll` sets `overflow-x`, which per spec computes `overflow-y` to `auto` as well, so a popup positioned inside a cell is clipped at the table's edge; a `position: fixed` child of `<body>` escapes that and any ancestor that later grows a `transform`. The event block reuses the calendar's three-tier colour language (amber `notable: true`, blue `"medium"`, plain otherwise) so the tiers read the same wherever they appear.
...
- `util/check_elections.py` — validates `docs/data/elections.yml` (see the Election dates section above). Local/offline, in CI and in the pre-commit hook (which runs it whenever that file is staged). Hard gates (exit 1): `NOT SOURCED` (no `url:`/`source:`), `NO PROOF` (no `quote:`/`note:`), `BAD DATE`, `BAD END DATE`, `BAD LEVEL`, `BAD DATE STATUS`, `BAD COUNTRY` (unknown to `calendar_export.py`'s `_COUNTRY_NAMES`), `DUPLICATE`, `UNKNOWN FIELD` (a typo'd key is silently dropped by every consumer, so the entry quietly loses whatever it meant). Soft warnings: `PAST` (held — replace it with the next one for that jurisdiction), `VAGUE SOURCE`, `NO DATE NOTE` (an `expected`/`deadline`/`month` date with nothing saying where it comes from), `STALE CHECK` (`url_checked:` missing or over 365 days old — the same `STALE_CHECK_DAYS` bar `check_event_sourcing.py` uses). It imports the level and date-status vocabularies *from the hook* rather than keeping a second copy, so a lint-clean entry can't render as something the calendar doesn't understand. The gate exists because the hook deliberately **skips** a malformed entry rather than crashing the build — without the linter, a typo'd date would silently vanish from the calendar with nothing said. `tests/test_check_elections.py` pins both halves.
  ```
  python util/check_elections.py                    # gate: exit 1 on any malformed/unsourced entry
  python util/check_elections.py --country AU        # scope to one country (repeatable)
  python util/check_elections.py --file other.yml    # check a different file (what the tests do)
  ```

- `util/discover_elections.py` — queries Wikidata (SPARQL) for upcoming elections and prints the ones `elections.yml` doesn't list yet. **Report-only by design — it never writes to `elections.yml`, and shouldn't be made to.** Wikidata is a *lead, not a source*, the same rule CLAUDE.md states for WebSearch/WebFetch output: checked against this corpus on 2026-09-02, its `point in time` for the 2027 NSW state election was 30 January 2027 — the earliest date the election could legally be held, not the 13 March the government announced (the fourth-Saturday default, 27 March, falls on Holy Saturday). A sync script would have written that into the calendar with a straight face. The same query also returned an item labelled "2022 Bosnian presidential election" dated 2026, several unlabelled Q-ids, and every result two or three times over (multiple `P31` paths reach one item). What it *is* excellent at is telling you an election **exists** — the 2026 Tasmanian local elections were missing until this surfaced them, which is the entire job. Note that an entry whose date disagrees with ours also shows up as "not in elections.yml", which is a useful second signal: go read the page.
  ```
  python util/discover_elections.py                      # Australia (default)
  python util/discover_elections.py --country NZ --country GB
  python util/discover_elections.py --all-countries      # noisy: by-elections, local races
  python util/discover_elections.py --until 2029-01-01
  python util/discover_elections.py --known              # also print what we already list
  ```

- `util/check_fragments.py` — mechanically re-verifies evidence against live pages for four sources through the same pipeline: (1) every event's `quote:` field, (2) every prose footnote's verbatim quoted excerpt (per the "Prose footnote citations" convention above), (3) every blog post's `shared_link.description:` (per the "Convention — shared_link" section above) checked against `shared_link.url`, and (4) every election's `quote:` in `docs/data/elections.yml` (per the Election dates section above) — scoped like shared links rather than like events, since an election belongs to no org: `--slug` (org-scoped) skips them, and `--elections-only` runs just those ~20 citations. All evidence shares the same cache (`docs/data/citation-state.json`, committed), fetch machinery, and AMBIGUOUS detection. The cache is keyed by URL; each URL's entry holds an `evidence` **list** (not a hash-keyed map — each item carries its own `id`, matching `citations.json`'s `evidence` array; `util/text_fragment.py`'s `find_evidence()` scans it, which is free in practice since a URL rarely has more than a handful of quotes). Each item carries `id` (= `sha256(normalize_ws(quote))`), the `quote` text, a `verified` (bot) and/or `manual_verified` (human) verdict, its own `checked` date, and a `context` (`prefix`/`text`/`suffix`/`sha256`), alongside URL-level fields (`checked`, `etag`, `last_modified`, `document_sha256`, `blocked`, `archive_url`, `url_status`, `manual_checked`). Network-dependent, so **not** wired into CI (matches this repo's fetch-then-cache convention of keeping the build offline) — instead runs report-only (`continue-on-error`, doesn't block the RSS/scrape commit) in the weekly `.github/workflows/heartbeat-probes.yml` cron alongside `check_rss.py`/`scrape_news.py`; check that workflow's log for findings. `--save-to-wayback` archives each URL to the Wayback Machine's Save Page Now service (no account needed); `--events-only` scopes verification to just events, skipping footnotes and shared links; `--unchecked-only` skips anything whose quote already carries a `verified` verdict in the evidence file's `evidence` map, with zero network calls for it (see the AMBIGUOUS/STILL BLOCKED bullets below for what the *default* cache-aware path still does per URL, even for something that hasn't changed). Honors `robots.txt` via `util/robots_check.py` before fetching any citation URL — a disallowed URL is recorded with `blocked: "ROBOTS_DISALLOWED"` in the same sticky cache 403/429 uses (see `check_evidence()`'s `BLOCKED_ERRORS`), skipped entirely on later runs. Wikipedia's own extracts API is deliberately not gated — it's designed for exactly this kind of programmatic access (see "Sourcing from Wikipedia" above).
  ...
  python util/check_fragments.py --elections-only  # only check docs/data/elections.yml's quotes
...
- `util/check_contact.py` — probes org websites for publicly-published email/phone/contact-form info and optionally writes them to `contact:` frontmatter. Fetches the homepage plus common contact-page paths (`/contact`, `/about`, `/get-involved`, …), respecting robots.txt. Two trust tiers: emails (`mailto:` links, Cloudflare-obfuscated `data-cfemail` attributes, or plain `@domain.tld`-shaped text) and detected public contact forms (a `<form>` with an email/message-shaped field, on a page whose URL reads as the contact page) are "high confidence" — **safe for `--write` to run unattended across the full org list**, since neither involves text-parsing guesswork. Phone numbers are the opposite: `tel:` links are high confidence, but phone-shaped digit sequences in plain text are "low confidence" and report-only, never auto-written — digit runs produce real false positives (dates, postcodes, prices) that `@domain.tld` text and `<form>` detection don't. Comparison-testing the email/tel: tier against org contact info already sourced by hand found it matched almost exactly; the disagreements were judgement calls (which of several valid published addresses is the right one to record — see the preference order in the Organisation pages `contact:` convention above) rather than parsing failures, so treat that choice, and any `--force` overwrite, as worth a spot-check in the resulting diff rather than blindly trusting every write. Existing `contact.email`/`contact.phone`/`contact.form` values are never overwritten unless `--force`. **Skips orgs checked within the last 180 days**, mirroring `check_rss.py`'s own default staleness gate (`docs/data/contact-check-state.json`, committed, keyed by slug — same "don't touch content frontmatter for pure probe state" pattern as `citation-state.json`, needed here because CLAUDE.md's own `contact:` convention says an org with nothing published gets no `contact:` block at all, so "checked, found nothing" has nowhere else to be remembered). `check_logo.py`'s "already has `logo:`" skip doesn't need this — its favicon-as-last-resort fallback means almost every org ends up with *some* `logo:` written, which is itself a skip signal for next time; contact info has no such fallback, and many small orgs genuinely publish no email/phone/form at all, so without a staleness gate every `just maintenance` run re-crawled all ~20 candidate paths on every org, every time, forever. `--force` bypasses the skip, for a single `--slug` or a full re-run. **A field that's already set is never overwritten without `--force` — but when a fresh finding disagrees with the existing value, that disagreement is flagged inline** (`[CONFLICTS with existing: ... — verify manually]`, and a `conflicts:` map in `--output` JSON) rather than silently discarded. No write happens either way in that case, so without the flag the disagreement would leave no `git diff` for anyone to ever notice. Phone conflicts are only flagged at high confidence (a `tel:` link) — the free-text phone regex's known false positives (dates, postcodes, prices) would otherwise drown genuine conflicts in noise. The point: full automation stays conservative (skip/keep on an unattended run), while a human or an LLM doing a deliberate review pass gets a concrete pointer to what's worth a second look, rather than having to cross-reference console output against frontmatter by hand. **`--skip-existing` goes a step further for that same reason**: it skips any org with an existing email/phone/form outright, regardless of the 180-day staleness gate — a re-check can surface a conflict that needs a human to look at, so it's only worth doing when someone's actually going to read the output. `just maintenance` passes `--skip-existing` (gap-filling only, safe to run and walk away from); `just maintenance-human-in-loop` drops it (still respects the 180-day gate, just doesn't take the "already has a record" shortcut) for when you intend to review the diff afterward.
  ```
...
**Quick-profile cards (the ℹ️ button).** Browsing the landscape used to cost a page load per org just to learn what one *is*, which is the wrong price for scanning 176 rows. Each Name cell carries an ℹ️ that opens a card with the org's `summary:`, its logo/type/country/status, a link to the full profile and its website, its last-activity date, and — the part that needs a rule — exactly one event, picked by `select_highlight()` in `hooks/org_events.py`:

- **Anything upcoming wins**, and among upcoming events the *soonest*, with `notable:` deliberately not consulted. A reader scanning the index is asking "is anything happening with this org?", and an org whose only upcoming entry is a routine meeting has answered that; skipping it to advertise a 2011 founding instead would not.
- **With nothing upcoming, `notable:` leads** — `true`, then `"medium"`, then untiered, each tier resolved by recency. That ordering is what the tiers are *for*: a founding or a landmark launch is the answer to "what was the last significant thing they did?" even when a routine check-in happened more recently. The card carries the org's last-activity date separately, so reaching back for a major event doesn't cost the reader the recency signal.
- `short_title:` is preferred over `title:` (this is exactly the one-line context it exists for), and `note:` over `quote:` for the blurb — with which one it is travelling in the data, since the template renders a quote as a blockquote rather than passing the source's own sentence off as DOD's summary. Blurbs are cut to `HIGHLIGHT_BLURB_CHARS` on a word boundary.

`tests/test_org_events.py` pins all of the above: the card shows one event out of up to a few dozen, so a wrong pick is invisible — the card still renders, just with the wrong thing on it.

Interaction notes worth knowing before touching the script: the card opens on hover (after a short delay, so it doesn't flash at every pointer crossing a row), on keyboard focus, and on click — a click *pins* it, which is the only path a touch device has, and also how a mouse user parks it open to click a link inside it. Two non-obvious guards: `Esc`/the close button refocus the trigger, and that refocus must not re-fire the trigger's own `focus` handler (or the card reopens instantly); the same dismissal also has to suppress *hover* until the pointer genuinely moves, because hiding a card that was covering its own trigger makes the browser re-fire `mouseenter` from a pointer that never moved. A filter change that hides the open card's row closes it too.

The card bodies are rendered once per org into a hidden `#org-info-sources` container placed **outside** the table, and cloned into the single shared card on demand. Outside, not in a `<td>`, because the search box filters on a row's `textContent` — parking summaries and event prose inside a row would silently widen what the search matches without showing the reader why a row matched. The cost is page weight: the index went from ~45 KB to ~101 KB gzipped (527 KB → 915 KB raw). That buys away a page load per org while browsing, which is the trade this feature exists to make, but it is the number to re-check before adding another field to the card.
