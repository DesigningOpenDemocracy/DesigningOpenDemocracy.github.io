...
- `logo: <path or URL>` — optional; the org's logo, shown on its own page, as a thumbnail in the `/organisations/` index table, and as the map marker in place of the plain type-icon pin. Path under `docs/assets/org-logos/<slug>.<ext>`, or a remote URL when a local copy isn't warranted. See `docs/assets/org-logos/README.md` for the sourcing/licensing convention (a per-logo table with License + Source columns) and `util/check_logo.py`, which can probe an org's site for a logo automatically. Rendered on a white backing card by default (so a transparent logo with dark content stays legible against the site's dark `slate` theme) — see `logo_bg` below for the inverse case.
  - `logo_bg: dark` — optional. Set this when the logo itself is white/light-coloured on a transparent background (it would disappear against the default white backing card) — swaps to a dark backing instead. Confirmed needed for e.g. CAfSA's white icon-on-transparent logo.
- `banner: <path or URL>` — optional; use instead of (not alongside) `logo:` display when the org's mark is a wide wordmark better shown full-width across the top of the page than boxed in the corner. Same sourcing convention as `logo:`. `logo:` is still required separately for the map marker/index table, which always render at icon size regardless of `banner:`.
