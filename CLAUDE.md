...
- `hooks/citation_export.py` — fires on `on_pre_build`; exports all event and footnote citations (`quote:` + `url:`) to `/data/citations.json` in CSL-JSON format with evidence array: `content-sha256` at URL level, `evidence[].type` / `evidence[].quote` / `evidence[].last-verified` per claim. Reads `docs/data/event-evidence-cache.json` for hashes and verified dates. See `internal-heartbeat/dod-citation-standard.md` for the design.
