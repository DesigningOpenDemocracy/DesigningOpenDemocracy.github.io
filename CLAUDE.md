...
- **`docs/data/citations.json`** (see Data exports below) reflects the same two fields — via the real CSL-JSON `archive`/`archive_location` fields, plus a DOD extension field `url-status` — as a **read-only projection** generated fresh by `hooks/citation_export.py` on every build. It never carries these three fields forward from its own previous output, specifically so a stale or since-corrected verdict in an old committed `citations.json` can't survive a rebuild that disagrees with the current evidence cache.
