...
- `events: [{date, title, url, source, note, quote, proof_level, proof_level_locked, proof_warning, url_checked, end_date, time, end_time, notable, type}]` — optional; a manually curated, editorial list of an org's significant milestones — **not** derived from `ics_feed:` and not meant to mirror it. `hooks/org_events.py` splits entries at build time into `page.meta.upcoming_events` (date >= today) and `page.meta.history_events` (date < today), rendered by `organisation.html` as two timeline sections. Keep each entry to one line — a terse `title`, no prose paragraph; if a milestone needs real narrative, put that in the page body instead (same judgment call as the optional "Key people" section). Future-dated entries here are also picked up by the site-wide calendar (see below) as a `manual`-source candidate alongside `ics_feed`-synced ones — no separate declaration needed. `end_date:` is optional, for events spanning more than one day (inclusive last day — same convention `util/sync_events.py` uses for iCal `DTEND`). `notable: true` is optional, for events significant enough to warrant the calendar's highlighted "major event" styling — use sparingly, it's meant to stand out.
  - **`type:`** — optional; a free-text tag (e.g. `conference`, `workshop`, `meetup`, `meeting`, `assembly`, `lecture`, `launch`, `rally`, `milestone`, `deadline`) rendered as an icon + label badge, both on the org's own upcoming/history timeline (`organisation.html`) and on the site-wide calendar (`calendar.html`) — each template keeps its own small `type_icons` lookup (unrecognised values still render with a generic 📅 fallback icon, so a new tag never needs a code change to display, only to get its own icon). Use `type: deadline` for a *submission* deadline — a call for proposals/papers/speakers closing date for a conference org — as opposed to the conference itself: add a separate `events:` entry dated on the deadline (not the conference date) with a title like `"Call for proposals deadline — <Conference Name> <Year>"`, `url:` pointing at the CFP page, and `type: deadline`. Because any future-dated `events:` entry is already picked up by the site-wide calendar (see below), this is enough on its own to get the deadline onto the calendar as a heads-up to submit in time — no separate mechanism needed. Add it as soon as a CFP opens with a known deadline; don't backfill one for a CFP that has already closed.
