...
| `/data/citations.json` | CSL-JSON — per-URL entries with `content-sha256` and `evidence` array (`type`, `quote`, `last-verified`), plus `archive`/`archive_location`/`url-status` when recorded (a read-only projection of `citation-evidence.json` — see "Citation archival" above). **Gitignored, regenerated at every build** — same treatment as `events.json`/`calendar.ics` below, since it's now a strict projection of two committed sources (markdown quotes + `citation-evidence.json`) with zero information that isn't already versioned elsewhere. Was committed until 2026-08-22; stopped once that projection became lossless, at which point committing it was pure build-output noise. |
