...
- **`archive_sha256`** — a hash of the archived snapshot's *own* content (not the live page — that's `document_sha256`, see above), written by the same `--save-to-wayback` run, immediately after the snapshot URL is obtained. Hashes the snapshot's raw ("id_"-suffixed) replay — the archived bytes without Wayback's injected toolbar/banner — through the same extraction pipeline used for live pages, so it's directly comparable in kind to `document_sha256`. Deliberately hash-only, never the snapshot text itself, for the same copyright reason the evidence cache never stores any full page body. Unlike a live page, an archived snapshot's content shouldn't drift once taken — this is a stable reference, useful both to catch the rare case of the archived copy itself being corrupted or altered, and, if DOD ever needs to migrate to a different archive provider, to confirm a replacement snapshot is a faithful copy before trusting it in place of this one. `archive_url` and `archive_sha256` are independent fields: a snapshot can be found (and `archive_url` set) on a run where hashing it fails (a transient fetch error) — that leaves `archive_sha256` unset without touching or clearing any hash a prior successful run already recorded.
...
- **`docs/data/citations.json`** (see Data exports below; gitignored, regenerated at every build) reflects `archive_url`/`url_status` — via the real CSL-JSON `archive`/`archive_location` fields plus a DOD extension field `url-status` — and `archive_sha256` — via a DOD extension field `archived_document: {sha256}`, a sibling of `document: {sha256}` rather than nested inside it, since the two hash different resources (the archived copy vs. the live page) — as a **read-only projection** generated fresh by `hooks/citation_export.py` on every build. It never carries these fields forward from its own previous output on disk, specifically so a stale or since-corrected verdict from an earlier local build can't survive a rebuild that disagrees with the current evidence file.
...
| `/data/citations.json` | CSL-JSON — per-URL entries with `id` + `convergence.sha256` (both the full 64-char `sha256(URL)`), `type`, `URL`, `title`, and an `evidence` array (`id` + `convergence.sha256`, both `sha256(normalize_ws(quote))`, plus `type` and `quote`), plus `archive`/`archive_location`/`url-status`, `document` (`{sha256}` — hash of the extracted live-page text, resource-level integrity), and `archived_document` (`{sha256}` — hash of the Wayback snapshot's own extracted content, added 2026-08-23; a sibling of `document`, not nested inside it, since the two hash different resources and are expected to diverge as a cited site's content changes out from under an earlier snapshot) when recorded (a read-only projection of `citation-evidence.json` — see "Citation archival" above). Evidence's `status` (`MATCH`/`MISMATCH`), `last-verified`, `verified-by`, and `context` (`{sha256}` only) are projected from the same cache, added 2026-08-23 — absent on any quote `check_fragments.py` hasn't successfully checked yet, which the spec reads as "not yet verified". `verified-by` is deliberately omitted for quotes confirmed from a human's browser snapshot (`manual_verified`), since the spec reads its absence as a human claim rather than a mechanical check. `context` ships as its hash alone: the stored `prefix`/`suffix`/`text` are diagnostic payload a verifier doesn't need (it refetches the page anyway) and would grow this file ~89% while republishing other organisations' paragraphs in bulk. (This row previously described a `content-sha256` field that has never existed in this file's output.) **Gitignored, regenerated at every build** — same treatment as `events.json`/`calendar.ics` below, since it's now a strict projection of two committed sources (markdown quotes + `citation-evidence.json`) with zero information that isn't already versioned elsewhere. Was committed until 2026-08-22; stopped once that projection became lossless, at which point committing it was pure build-output noise. |
...
- `util/check_fragments.py` — mechanically re-verifies evidence against live pages for three sources through the same pipeline: (1) every event's `quote:` field, (2) every prose footnote's verbatim quoted excerpt (per the "Prose footnote citations" convention above), and (3) every blog post's `shared_link.description:` (per the "Convention — shared_link" section above) checked against `shared_link.url`. All evidence shares the same cache (`docs/data/citation-evidence.json`, committed), fetch machinery, and AMBIGUOUS detection. Network-dependent, so **not** wired into CI (matches this repo's fetch-then-cache convention of keeping the build offline) — instead runs report-only (`continue-on-error`, doesn't block the RSS/scrape commit) in the weekly `.github/workflows/heartbeat-probes.yml` cron alongside `check_rss.py`/`scrape_news.py`; check that workflow's log for findings. `--save-to-wayback` archives each URL to the Wayback Machine's Save Page Now service (no account needed) and hashes the resulting snapshot's own content (`archive_sha256` — see "Citation archival" above); `--events-only` scopes verification to just events, skipping footnotes and shared links; `--unchecked-only` skips anything already recorded in the evidence file's `verified` map, with zero network calls for it (see the AMBIGUOUS/STILL BLOCKED bullets below for what the *default* cache-aware path still does per URL, even for something that hasn't changed). Honors `robots.txt` via `util/robots_check.py` before fetching any citation URL — a disallowed URL is recorded with `blocked: "ROBOTS_DISALLOWED"` in the same sticky cache 403/429 uses (see `check_evidence()`'s `BLOCKED_ERRORS`), skipped entirely on later runs. Wikipedia's own extracts API is deliberately not gated — it's designed for exactly this kind of programmatic access (see "Sourcing from Wikipedia" above).
  ...
  - **`--unchecked-only`** — the *default* run still issues a conditional-GET request (ETag/If-Modified-Since) per URL every time, even for evidence that's never changed and never will (a 304 is cheap on bandwidth, but it's still one request per citation, every run, forever). `--unchecked-only` skips that entirely for anything already present in the evidence file's `verified` map — no request at all, not even a conditional one — and only fetches evidence that's genuinely new: a citation added since the last run, or a new `quote:`/footnote/`shared_link.description:` on an already-known URL. A prior MISMATCH still counts as "checked" and is skipped too — this flag is for catching up quickly on new evidence, not for re-litigating known failures (drop the flag, or use `--no-cache`, for that — the two can't be combined, since they pull in opposite directions on how much to trust the evidence file).
  ```
...
- `util/manual_dump.py` / `util/import_manual_dump.py` — an escape hatch for citation URLs that stay unreachable to every automated path (bot protection, rate limits — both the origin site's own and Wayback Machine's Save Page Now — or robots.txt) but a human's own browser can still load fine. `manual-dump/` lives at the repo root, entirely gitignored — local working state, not archival; the durable, shareable copy of a citation is still Wayback Machine (`check_fragments.py --save-to-wayback`), not this.
  ...
  **Maintainer runbook — steps to actually do this:**
  ...
  5. `git diff docs/data/citation-evidence.json` and skim the new `manual_verified` entries before committing — same spot-check judgment as reviewing any other automated write to this file.
