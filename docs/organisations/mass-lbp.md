---
title: MASS LBP
type: practice
status: active
country: CA
website: https://www.masslbp.com
logo: /assets/org-logos/mass-lbp.webp
contact:
  checked: 2026-07-30
  email: hello@masslbp.com
  source: https://www.masslbp.com/about
summary: A Canadian civic design firm specialising in citizens' assemblies, civic
  lotteries, and deliberative public engagement — with over 50 reference panels completed
  for governments and institutions across North America.
concepts:
- deliberative-democracy
- citizens-assembly
- sortition
location:
  latitude: 43.6532
  longitude: -79.3832
  name: Toronto, Canada
  precision: city
events:
- date: '2007-01-01'
  title: Founded by Peter MacLeod
  url: https://www.masslbp.com/about
  quote: Since its founding in 2007, MASS has completed more than 200 major policy
    projects for governments and public agencies across Canada
  proof_level: high
  url_checked: '2026-08-10'
  notable: true
- date: '2009-01-01'
  title: Ran the Central LHIN Citizens' Reference Panel on Regional Health Priorities
  url: https://www.masslbp.com/work-panels
  quote: Central LHIN Citizens’ Reference Panel on Regional Health Priorities
  proof_level: high
  url_checked: '2026-08-10'
  notable: false
activity:
  manual:
    checked: 2026-06-07
    date: 2023-08-14
    note: website loaded. Uncertain if active. Last social media post at Aug 14, 2023
      'We’re aiming to expand our team this fall by adding a new Coordinator at MASS.
      We are looking for a candidate with the right mix of qualities, skills and experience
      to help us build our practice and raise our game.'
  social:
    date: 2025-07-21
    note: 'Latest video: What is MASS LBP?'
    url: https://www.youtube.com/watch?v=x-WbnLvrBBc
  sitemap:
    checked: 2026-08-17
    date: 2026-05-22
    note: Page last modified (from sitemap)
last_checked: '2026-05-29'
---

MASS LBP is a Toronto-based civic design firm founded in 2007 that specialises in designing and running citizens' assemblies, civic lotteries, and deliberative public engagement processes. It takes its name from the French phrase *masse de manœuvre* — a reserve force held back for decisive deployment.

MASS LBP has completed over 50 "reference panels" (their term for mini-publics) for governments, institutions, and organisations across Canada and the United States. Their work spans planning, policy, infrastructure, healthcare, and governance reform.

Key projects include:

- **Ontario Citizens' Reference Panel on Electoral Reform** (2007) — one of the first large-scale citizens' assemblies in Canada, recommending Mixed Member Proportional representation
- Multiple reference panels for the **City of Toronto** on infrastructure priorities and civic issues
- Work with **Infrastructure Ontario**, **Waterfront Toronto**, and other major public bodies
- **Better Transit Citizens' Reference Panel** — engaging Torontonians on transit priorities

MASS LBP's methodology draws heavily on sortition — random selection to create a representative cross-section of the public — combined with structured deliberation, expert testimony, and facilitated small-group discussion. Their practice emphasises the quality of the deliberative process over the quantity of participants.

## Links

- Website: [masslbp.com](https://www.masslbp.com)

## See also

- [Deliberative Democracy](../concepts/deliberative-democracy.md)
- [Citizens' Assembly](../concepts/citizens-assembly.md)
- [Sortition](../concepts/sortition.md)
- [newDemocracy Foundation](newdemocracy.md)
- [Sortition Foundation](sortition-foundation.md)
- [Healthy Democracy](healthy-democracy.md)
- [MosaicLab](mosaiclab.md)
- [DemocracyCo](democracyco.md)