---
title: Flux Party
type: political-party
country: AU
website: https://voteflux.org/
summary: "An Australian political party that uses Issue-Based Direct Democracy (IBDD) to let constituents direct how elected representatives vote in parliament."
contributors:
  - Charlie
---

![Flux Party logo](flux-mark.svg)

Flux is an Australian political party built around the concept of Issue-Based Direct Democracy (IBDD). Rather than a representative voting on their own judgement, Flux aims to have elected members vote in parliament according to the aggregated preferences of their constituents, as expressed through a digital platform.

The party was founded by Max Kaye and Nathan Spataro. It has fielded Senate candidates in multiple federal elections and has also operated at the state level. The AEC deregistered the federal party in 2022 due to a decline in registered membership, though state branches have continued activity.

## How IBDD Works

Constituents use the Flux app to vote on policy issues. Each user is allocated a number of votes to spend across issues they care about — they can vote directly, delegate their vote to someone else on a specific topic, or abstain and save their votes for later issues. An elected Flux member would then vote in parliament to reflect the outcome of this process.

## Links

- Website: [voteflux.org](https://voteflux.org/)
- Wikipedia: [Flux (political party)](https://en.wikipedia.org/wiki/Flux_(political_party))

## Podcast

Designing Open Democracy recorded a podcast episode with Ben Ballingall, Victoria representative of the Flux Party, on 2020-02-11.
[Listen on Apple Podcasts](https://podcasts.apple.com/au/podcast/talk-ben-ballingall-about-flux-party-issue-based-direct/id1492656241?i=1000465446730)
