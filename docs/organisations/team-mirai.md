---
title: Team Mirai
type: party
status: active
country: JP
website: https://team-mir.ai
logo: /assets/org-logos/team-mirai.png
contact:
  checked: 2026-08-09
  form: https://team-mir.ai/contact
  source: https://team-mir.ai
summary: A Japanese political party founded in 2025 by AI engineer Takahiro Anno,
  advocating e-democracy and digital democracy — using technology platforms to enable
  direct citizen input into legislative processes. Won 1 Upper House seat in 2025
  and 11 Lower House seats in the February 2026 general election.
concepts:
- democracy-tools
- e-government
- liquid-democracy
- direct-democracy
location:
  latitude: 35.6762
  longitude: 139.6503
  name: Tokyo, Japan
  precision: city
events:
- date: '2025-05-08'
  title: Party officially established by AI engineer Takahiro Anno
  url: https://en.wikipedia.org/wiki/Team_Mirai
  quote: was established on 8 May 2025
  proof_level: high
  url_checked: '2026-08-10'
  notable: true
  notable_reason: Founding of the organisation
- date: '2025-07-20'
  title: Wins 1 seat in House of Councillors with 1.5 million votes (2.6%)
  url: https://en.wikipedia.org/wiki/Team_Mirai
  quote: won its first seat in the Diet
  proof_level: high
  url_checked: '2026-08-10'
  notable: true
  notable_reason: Election result
- date: '2026-02-08'
  title: Wins 11 seats in House of Representatives with 3.8 million votes (~6.9%)
  url: https://en.wikipedia.org/wiki/Team_Mirai
  quote: securing 11 seats in the House of Representatives
  proof_level: high
  url_checked: '2026-08-10'
  notable: true
  notable_reason: Election result
activity:
  dod:
    date: 2026-09-19
    note: Site live, confirmed active (11 lower house seats, nationwide caravan 2026
      underway)
    url: https://team-mir.ai
    checked: 2026-09-19
  rss:
    note: No feed found
    checked: 2026-09-25
last_checked: '2026-09-19'
---

Team Mirai (チームみらい, "Team Future") is a Japanese political party founded in May 2025 by Takahiro Anno, an AI engineer and entrepreneur. It frames itself as a technology-native political force: the mission is to use technology to make politics more transparent, efficient, and accessible — replacing opaque processes with open ones, and closing the distance between citizens and the decisions that shape their lives.

The party won Anno a seat in the upper house in July 2025 and then 11 seats in the lower house in February 2026 (~6.9% of the proportional-representation vote, 3.8 million votes). Its parliamentary delegation sits on the Diet's Special Committee on Formation of Digital Society and AI Utilization. Team Mirai has attracted international attention — coverage in The Diplomat, the New York Times, Japan Times, and Tech Policy Press, and commentary from Bruce Schneier — as a live experiment in whether a party explicitly organised around digital-democratic infrastructure can compete and govern. The party publishes all political funding flows openly, accepts only individual donations, and has built its own transparency tools (including an AI-powered policy Q&A system).

The Plurality framework, developed by RadicalxChange founder Glen Weyl and colleagues, is a cited influence. Team Mirai is referenced in the [RadicalxChange entry](radicalxchange.md) as an example of Plurality ideas entering parliamentary politics.

## Links

- Website: [team-mir.ai](https://team-mir.ai)

## See also

- [Democracy Tools](../concepts/democracy-tools.md)
- [E-Government](../concepts/e-government.md)
- [Liquid Democracy](../concepts/liquid-democracy.md)
- [RadicalxChange](radicalxchange.md)