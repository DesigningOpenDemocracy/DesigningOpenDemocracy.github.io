---
title: Fusion Party
type: party
status: active
country: AU
website: https://www.fusionparty.org.au
summary: An Australian political party formed in 2021 from the merger of five minor
  parties (Science, Pirate, Secular, Vote Planet, Climate Change Justice), retaining
  them as semi-autonomous internal branches. Platform includes political donation
  disclosure, an anti-corruption commission, whistleblower protections, and lowering
  the voting age to 16.
concepts:
- radical-transparency
location:
  latitude: -37.05
  longitude: 146.0833
  name: Mansfield, Victoria, Australia
  precision: city
related_orgs:
- pirate-party-australia
events:
- date: '2021-01-01'
  title: Formed via merger of the Science, Pirate, Secular, Vote Planet, and Climate
    Change Justice parties, after new AEC rules raised the minimum party membership
    threshold from 500 to 1,500
  url: https://en.wikipedia.org/wiki/Fusion_Party_(Australia)
  quote: was formed in 2021 following the passing of the Electoral Legislation Amendment
    (Party Registration Integrity) Bill 2021
  proof_level: high
  url_checked: '2026-08-15'
  notable: true
  notable_reason: Merger of organisations
activity:
  dod:
    checked: 2026-08-15
    date: 2026-08-15
    note: Wikipedia and party site confirm active registration; fielded candidates
      in the 2025 federal election
    url: https://en.wikipedia.org/wiki/Fusion_Party_(Australia)
  sitemap:
    date: 2026-08-28
    note: "Page last modified (from sitemap)"
    url: https://www.fusionparty.org.au/sitemap_index.xml
    checked: 2026-08-28
last_checked: '2026-08-15'
---

Fusion is a federated Australian political party formed in 2021, when the Science Party, Pirate Party, Secular Party, Vote Planet, and Climate Change Justice Party merged in response to legislation that raised the minimum party membership threshold from 500 to 1,500 members. Rather than dissolving into a single organisation, the founding parties retained a degree of autonomy as formal internal branches — [Pirate Party Australia](pirate-party-australia.md) is the most active of these on democracy-reform issues specifically.

The party's platform covers a broad range of policy areas (climate, secularism, drug law reform) but includes a substantial governance-reform component: disclosure of political donations above $1,000, open access to the advice behind policy decisions, an anti-corruption commission, protections for whistleblowers and activists, a constitutional bill of rights, and lowering the voting age to 16 (while keeping the compulsory voting age at 18) alongside electoral education in high schools.

Fusion has contested federal elections in 2022 and 2025 without winning a seat, and changed its registered name to "FUSION | Planet Rescue | Whistleblower Protection | Innovation" in January 2025.

## Links

- Website: [fusionparty.org.au](https://www.fusionparty.org.au)

## See also

- [Radical Transparency](../concepts/radical-transparency.md)
- [Pirate Party Australia](pirate-party-australia.md)
