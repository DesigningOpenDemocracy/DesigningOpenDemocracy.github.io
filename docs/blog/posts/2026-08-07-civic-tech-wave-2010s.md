...
Worth naming plainly: this site is also a product of the same moment. Designing Open Democracy held its first meetup in December 2016 — the same year MiVote launched, a year after Flux was founded, with Loomio, DemocracyOS, and vTaiwan already running. That timing shaped what DOD became. A premise some in the group have carried from early on — usually said out loud when someone asks why DOD exists, rarely written down — is that there's nothing new under the sun in this space: most reform ideas being tried have been tried before, somewhere, and the people building the next version are often unaware the lesson already exists. On that view DOD isn't a democracy-reform group competing with the ones catalogued above; it's closer to a meta-organisation, paying attention across the whole space so the lesson doesn't have to be re-learned from scratch each time. It's a premise held by a few people in the group rather than a settled institutional position, and [we've only just started asking in public whether it actually holds up](../../philosophy/index.md).

So instead of building a voting platform or running for office, DOD took a third path: keep discussing, evaluating, and comparing what everyone else was building. [Nick Merange's 2018 comparative evaluation of Flux, MiVote, Online Direct Democracy, and citizens' juries](2018-05-03.md) — with [Max Kaye's response](2018-05-03-Max.md) alongside it — is, structurally, the same exercise as this post: DOD doing this same reckoning eight years earlier, in real time, while its subjects were still active.

There's a harder-edged part of that same premise worth stating plainly: a fair reading of the base rate, even at the time, was that most of what's catalogued above should have been expected to fail — and not always for reasons that reflect badly on the effort itself. Flux and MiVote built more structurally sound binding-mandate mechanisms than anything else in this piece and still won zero seats between them. That's not obviously evidence IBDD or the four-destination model was wrong; it looks more like evidence that no minor party breaks into the Senate on governance-design quality alone, regardless of how well the mechanism works. Seeing that coming doesn't take cynicism about any one project. It takes watching the base rate across the whole space rather than getting attached to any single bet within it.

DOD's own answer to that base rate is the opposite of what most of the organisations above tried. Instead of a costly project chasing institutional adoption or an electoral seat, the strategy has been to stay cheap enough to keep going regardless of whether any particular bet pays off. A discussion forum has close to zero burn rate compared with fielding Senate candidates or maintaining a voting platform against a shrinking team. Persistence, not ambition, is the actual strategy — a less dramatic answer to "why has DOD lasted" than any of the survival stories above, but arguably the more honest one.

Two things keep that from reading as smugger than it should. First, DOD's whole role depends on other people being willing to take the costly path it doesn't. Without Flux and MiVote actually attempting the binding-mandate problem, there'd be nothing to learn about minor parties and governance quality; without vTaiwan actually running inside a ministry for three years, there'd be no accountability-sink gap to point at. A landscape made entirely of organisations optimising for cheap persistence would have nothing left to be a meta-organisation about — DOD needs the doers more than they need DOD. Second, and more plainly: DOD isn't necessary either. The ecosystem doesn't require a meta-organisation to function — the lessons above got written up by journalists and academics (see the Noveck and Chen citations earlier in this post) whether or not anyone was cataloguing the pattern, and they'd surface eventually regardless. What a group like this adds, if it adds anything, is making the lesson a bit easier to find and a bit harder to forget for whoever's building the next version. That's a modest claim, on purpose.

That path is a plausible reason DOD is still here to write this retrospective while most of the organisations in it aren't.
