...
**Caught lapse #2 (July 2026, applied outside index.md itself):** While adding three media-accountability organisation pages to the Democracy Landscape (Constitutional Clarion, Lamestream, Ette Media — a new `type: media` precedent for solo/founder-led "landscape of one" entries), one page's account of a News Corp sting on a small Sydney restaurant closed on "both the Telegraph and the restaurant apologised" without quoting either party's own characterisation of what had actually been done to them. The human founder's correction wasn't "add a disclaimer" — it was a reminder that DOD's standard is to not dodge controversial topics, be careful instead, and lean toward human rights; historically that lean has not always been the comfortable, peaceful choice. Concretely: journalism's job is to go outside and check if it's raining, not to report that both sides have a point. The fix was to source and quote the smaller party's own Federal Court characterisation of the incident (a "cruel experiment") rather than let a real settlement's mutual-apology framing read as if the dispute were symmetrical. Same family of bias as the entry above — flattening toward a comfortable-sounding neutrality reads as "careful" but is actually a subtler failure to verify and attribute specifically. Worth a session doing Democracy Landscape curation work noticing: the discipline this document asks for doesn't stop at `index.md`; it's a habit of drafting, not a checklist applied once per file.
...
Last updated: July 2026 (second caught-lapse entry, from Democracy Landscape curation work rather than an `index.md` edit — see Claude's section). Previously: July 2026 (reflexive-application addition to "Relative epistemology," prompted by a caught inconsistency in a heartbeat post); June 2026 (relocation to `docs/projects/accountability-framework/`).
